...
///	private boolean[] allBoardsXHasDoubleWin = null;
///	private boolean[] allBoardsOHasDoubleWin = null;
	private GeneticAlgorithm[] gax;
	private GeneticAlgorithm[] gao;
	private Random rand = new Random();
	private boolean initialized = false;

	public GeneticPlayer2() {
		...
		public MyFitnessFunction
			...
			Player gPlayer = new Player() {
				public int chooseNextMove(char[][] grid, char whichPlayer) {
					...
					return GeneticPlayer2.this.chooseNextMove(grid, whichPlayer);
				}
			};
...
///
/*
	private static boolean playerHasDoubleWin(char[][] grid, char whichPlayer) {
		int r, c, rc, pc, sc, src;
		boolean[][] isWinSpace = new boolean[3][3];
		...
		if ( (pc == 2) && (sc == 1) ) {
			isWinSpace[src][2-src] = true;
		}

		int numWinSpaces = 0;
		for (r = 0; r < 3; r++) {
			for (c = 0; c < 3; c++) {
				if (isWinSpace[r][c]) numWinSpaces++;
			}
		}

		return (numWinSpaces >= 2);
	}
*/

	public void train(int freeCount) {
		if (allBoards == null) {
...
///			allBoardsXHasDoubleWin = new boolean[allBoards.length];
///			allBoardsOHasDoubleWin = new boolean[allBoards.length];
...
///				allBoardsXHasDoubleWin[boardIdx] = playerHasDoubleWin(allBoards[boardIdx], 'X');
///				allBoardsOHasDoubleWin[boardIdx] = playerHasDoubleWin(allBoards[boardIdx], 'O');
...
///
System.out.println("learn "+freeCount);

		int numGames = 1;
		for (int fc = freeCount-1; fc > 0; fc -= 2) numGames *= fc;

		int maxNoImprovmentsPasses = 50;

		double oldFitness;
		int noImprovementsPasses = 0;
		while (true) {
System.out.println(" learning freeCount="+freeCount+" noImprovementPasses="+noImprovementsPasses);
			...
			for (int boardIdx = 0; boardIdx < allBoards.length; boardIdx++) {
				if ((9-(allBoardsXMoves[boardIdx]+allBoardsOMoves[boardIdx])) == freeCount) {
					if (gax[boardIdx] != null) {
						oldFitness = gax[boardIdx].getIndividualFitness(0);
						if (oldFitness < numGames) {
							...
							anyTrained = true;
///
///System.out.println(""+freeCount+" x "+gax[boardIdx].getIndividualFitness(0)+" "+numGames);
							if (gax[boardIdx].getIndividualFitness(0) > oldFitness) {
								anyImproved = true;
							}
						}
					}
					if (gao[boardIdx] != null) {
						oldFitness = gao[boardIdx].getIndividualFitness(0);
						if (oldFitness < numGames) {
							...
							anyTrained = true;
///
///System.out.println(""+freeCount+" o "+gao[boardIdx].getIndividualFitness(0)+" "+numGames);
							if (gao[boardIdx].getIndividualFitness(0) > oldFitness) {
								anyImproved = true;
							}
						}
					}
			...
			if (anyImproved) {
				noImprovementsPasses = 0;
			} else {
				noImprovementsPasses++;
			}
			...
			if (noImprovementsPasses >= maxNoImprovmentsPasses) {
				break;
			}
		}
	}
